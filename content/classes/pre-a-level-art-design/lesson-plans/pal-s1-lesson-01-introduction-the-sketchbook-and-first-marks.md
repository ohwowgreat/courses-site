---
title: "PAL S1 Lesson 01: Introduction, the Sketchbook and First Marks"
tags: [pre-a-level, grade-10, lesson-plan, semester-1]
related:
  - "[[classes/pre-a-level-art-design/pre-a-level-art-design]]"
  - "[[classes/pre-a-level-art-design/unit-plans/pal-s1-unit-1-drawing-and-the-visual-elements]]"
  - "[[classes/pre-a-level-art-design/unit-plans/pal-semester-1-plan]]"
  - "[[classes/pre-a-level-art-design/assessments/pal-s1-assessments]]"
  - "[[classes/pre-a-level-art-design/pal-resource-library]]"
  - "[[classes/pre-a-level-art-design/lesson-plans/pal-s1-lesson-02-tone-and-form]]"
modified: 2026-07-20
---
<figure class="plate plate--hero">
  <img src="../../../static/img/first-marks.jpg" alt="Ilya Repin, Leo Tolstoy at Work, 1891" />
  <figcaption>Ilya Repin, <em>Leo Tolstoy at Work</em>, 1891</figcaption>
</figure>


# PAL S1 Lesson 01: Introduction, the Sketchbook and First Marks

[[classes/pre-a-level-art-design/pre-a-level-art-design|Pre A Level Art & Design]] · [[classes/pre-a-level-art-design/unit-plans/pal-s1-unit-1-drawing-and-the-visual-elements|Unit 1]] · U1·L1 · [[classes/pre-a-level-art-design/lesson-plans/pal-s1-lesson-02-tone-and-form|L02]] →

## At a glance

| | |
|---|---|
| Session | W1, one Wednesday double (~90 min) |
| Dates | Wed 2026-09-02 |
| Focus | Course opening; the sketchbook contract; the visual elements; line and mark |
| Deliverable | Introduction Survey completed (incl. the garlic drawing); sketchbook set up; first line studies |
| Feeds | The survey baselines the mid-term narrative and carries the U4 demand signal; the sketchbook regime runs to A5 (Wed 2027-01-06) |
| Homework | Finish the *Drawing Exercises 1* mark sheets; decorate the sketchbook cover; post the session's pages on MB (the weekly Learning Behaviour posting) |

## Goal

Three things must be true by the end of the double: students know what the course is and how it grades; every student owns a working sketchbook and knows the contract attached to it; and drawing has already started — the session ends with marks on paper, not with administration. The garlic drawing on the survey is the semester's day-one baseline, made before any teaching happens.

## How it runs

**First half — the opening (~45 min)**, per first session opener, plain language for the G10 ESL room:

- **Welcome and the course (10 min)** — who I am, who they are, what the four units make (drawing → photography → collage → poster), how the grade is built (Attainment 50 / Course Skills 20 / Learning Behaviour 10 / End of Term 20 — one slide, no detail yet).
- **Introduction Survey (15 min)** — the two-page survey: favorites, preferred area of study, the 1–10 skill self-ratings, the two open questions, then the garlic-bulb drawing ("draw this any way you like"). Say clearly: this is not a test, nobody fails a survey.
- **The sketchbook (15 min)** — hand out or check sketchbooks; the contract, stated once and posted on MB: **2–3 pages every week**, checked twice for a Course Skills mark (**SB1 Wed 10-21, SB2 Wed 12-02**), submitted whole at the end as **A5 (Wed 2027-01-06)**; plus a photo of this week's pages posted on MB after every session. Cover decoration starts now, finishes at home.
- **Questions (5 min).Second half — U1·L1: the visual elements, starting with line (~45 min):**

- **The elements (10 min)** — the classroom element chart: line, shape, color, value, texture, space, form. One sentence each; this unit works through them from observation.
- **Blind contour (10 min)** — two 3-minute blind-contour drawings of a partner's hand or their own shoe: eyes on the subject, pen never lifts, no looking at the page. Expect laughter; the point is looking, not likeness.
- **Continuous line (10 min)** — same subject, now looking allowed, still one unbroken line. Compare the pair: which one *looked* harder?
- **The mark grids (10 min)** — start *Drawing Exercises 1*: copy the hard-pencil (6H–2H) and soft-pencil (2B–6B) mark grids into the sketchbook. Started in class so the format is understood; finished at home.
- **Close (5 min)** — sketchbooks open on tables, one-minute walk-through; homework stated and posted on MB.

## Assessment

Nothing is graded this session. The survey is a baseline instrument, not a mark. The sketchbook regime announced today is the standing notice for **A5** — the whole semester's book, submitted Wed 2027-01-06 — so notice is never a constraint on it. Full register: [[classes/pre-a-level-art-design/assessments/pal-s1-assessments|PAL S1 Assessments]].

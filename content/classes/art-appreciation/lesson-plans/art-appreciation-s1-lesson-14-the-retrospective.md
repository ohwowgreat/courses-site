---
title: "Art Appreciation S1 Lesson 14: The Retrospective"
tags: [art-appreciation, lesson-plan, semester-1]
related:
  - "[[classes/art-appreciation/art-appreciation]]"
  - "[[classes/art-appreciation/unit-plans/art-appreciation-s1-unit-5-semester-close-and-retrospective]]"
  - "[[classes/art-appreciation/lesson-plans/art-appreciation-s1-lesson-13-the-artworld-and-the-brief]]"
  - "[[classes/art-appreciation/lesson-plans/art-appreciation-s1-lesson-15-return]]"
modified: 2026-07-19
---

# Art Appreciation S1 Lesson 14: The Retrospective

[[classes/art-appreciation/art-appreciation|Art Appreciation]] · [[classes/art-appreciation/unit-plans/art-appreciation-s1-unit-5-semester-close-and-retrospective|Unit 5]] · Lesson 14 of 15 · ← [[classes/art-appreciation/lesson-plans/art-appreciation-s1-lesson-13-the-artworld-and-the-brief|L13]] | [[classes/art-appreciation/lesson-plans/art-appreciation-s1-lesson-15-return|L15]] →

## At a glance

| | |
|---|---|
| Session codes | `U5·L1`–`U5·L5` |
| Days | 5 |
| Dates | Mon 2027-01-04 to Fri 2027-01-08 |
| Template | PRODUCTION |
| Deliverable | The semester-close constellation retrospective — End of Term, 20% |
| Feeds | Semester 2 |
| Homework | None |

## Goal

The whole semester, read back as one argument. The retrospective is the End of Term category, sat in class on day four rather than in the exam window — "a decision that sits outside the handbook's expectation and is flagged in the workbook as open." This is the third constellation of the year and "the one where the method finally looks like a method rather than a rule." Everything they have made is available; the Unit 1 boards and the Unit 4 syntheses go up on the walls before Monday.

## How it runs

- **Day 1 · Mon 4 Jan · `U5·L1`** — inventory: everything on the wall, 35 minutes, **no arranging yet**.
- **Day 2 · Tue 5 Jan · `U5·L2`** — the three works from the break homework, defended to someone who chose differently — "and does not just concede."
- **Day 3 · Wed 6 Jan · `U5·L3`** — constellation retrospective workshop: test it while it can still change. "This is the practice and it is the last chance."
- **Day 4 · Thu 7 Jan · `U5·L4`** — **RETROSPECTIVE sits.** In class, 40 minutes. End of Term, 20%.
- **Day 5 · Fri 8 Jan · `U5·L5`** — read the room's constellations; name the question you want Semester 2 to answer. Last teaching day of the semester.

## Assessment

- **EoT retrospective sits Tue 2027-01-05**; criteria issued with the announcement. Marked on showing a semester of looking as one argument, with the argument in the layout.
- Sat outside the exam window (Mon 2027-01-11 to Mon 2027-01-18) on the teacher's stated decision — open item, confirm with department.

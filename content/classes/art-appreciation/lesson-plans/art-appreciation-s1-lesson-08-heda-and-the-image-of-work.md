---
title: "Lesson 08: Heda and the Image of Work"
related:
  - "[[classes/art-appreciation/art-appreciation]]"
  - "[[classes/art-appreciation/unit-plans/art-appreciation-s1-unit-3-food-the-table-and-ethics]]"
  - "[[classes/art-appreciation/lesson-plans/art-appreciation-s1-lesson-07-the-split-weeks]]"
  - "[[classes/art-appreciation/lesson-plans/art-appreciation-s1-lesson-09-food-the-table-and-ethics]]"
modified: 2026-07-26
---
# Lesson 08: Heda and the Image of Work

[[classes/art-appreciation/art-appreciation|Art Appreciation]] · [[classes/art-appreciation/unit-plans/art-appreciation-s1-unit-3-food-the-table-and-ethics|Unit 3]] · Lesson 08 of 15 · ← [[classes/art-appreciation/lesson-plans/art-appreciation-s1-lesson-07-the-split-weeks|L07]] | [[classes/art-appreciation/lesson-plans/art-appreciation-s1-lesson-09-food-the-table-and-ethics|L09]] →

<p class="handouts"><strong>Lesson slides:</strong> <a href="../../../decks/art-appreciation/art-appreciation-s1-lesson-08-heda-and-the-image-of-work.pptx">Download the deck (PowerPoint, 2.5 MB)</a> · <a href="../../../decks/credits">image credits</a></p>

## At a glance

| | |
|---|---|
| Session codes | `U3·L11`–`U3·L17` |
| Days | 7 teaching days across 10 calendar days |
| Dates | Mon 2026-11-02 to Fri 2026-11-13 (G11 out Wed 11-04 to Fri 11-06 for Juniors Days) |
| Deliverable | CS4 Heda paragraph, plus LB5 |
| Feeds | A3 |
| Homework | Reading on Dutch still life and the inventory, 30 min, set day 4, due day 6 |

These dates may shift when the school publishes the calendar for this period.

## Objectives

By the end of this lesson, you can:

1. Read a still life as a list of possessions rather than as an arrangement
2. Find the moment Leonardo chose, and say why that one and not the one before it
3. Write one paragraph in which every adjective is earned

## How it runs

This runs as one lesson across a Monday–Tuesday start, a gap while G11 is out for Juniors Days, and then a full week.

- **Day 1 · Mon 2 Nov · `U3·L11`** — Heda: the still life as inventory. Name every object and who owns it.

<figure class="plate">
  <img src="../../../static/img/heda-still-life.jpg" alt="Willem Claesz. Heda, Still Life with a Gilt Cup, 1635" loading="lazy" />
  <figcaption>Willem Claesz. Heda, <em>Still Life with a Gilt Cup</em>, 1635</figcaption>
</figure>

- **Day 2 · Tue 3 Nov · `U3·L12`** — A3 (Unit 3 essay, due Thu 19 Nov) is announced in the first ten minutes. Then Heda again, the same painting, alone this time, further than yesterday. LB5 posted. Then three days out.
- **Day 3 · Mon 9 Nov · `U3·L13`** — Leonardo, *The Last Supper*: find the moment Leonardo chose, and say why that one and not the one before it.
- **Day 4 · Tue 10 Nov · `U3·L14`** — Bruegel and Steen: the disorderly table, taken seriously as a subject rather than comedy.
- **Day 5 · Wed 11 Nov · `U3·L15`** — the Qingming scroll, unrolled, followed by food along its length — 35 of the 45 minutes.
- **Day 6 · Thu 12 Nov · `U3·L16`** — the moral image of work: when did work become worth painting, and for whom.
- **Day 7 · Fri 13 Nov · `U3·L17`** — write the Heda paragraph; swap and cut one adjective from someone else's. CS4 collected. This is direct practice for A3.

## Assessment

- **A3** — announced Tue 2026-11-03, the last session both cohorts attend before the deadline, giving 16 days' notice. Essay due Thu 19 Nov.
- **LB5** — posted Tue 2026-11-03, from the second Heda session.
- **CS4** — due Fri 2026-11-13, the Heda paragraph: one paragraph that earns every adjective in it.

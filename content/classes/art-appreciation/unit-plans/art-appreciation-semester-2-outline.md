---
title: Art Appreciation Semester 2 Outline
tags: [art-appreciation, semester-2, unit-plan, outline, 2026-27]
related:
  - "[[classes/art-appreciation/art-appreciation]]"
  - "[[classes/art-appreciation/unit-plans/art-appreciation-semester-1-plan]]"
  - "[[shared/school-academic-calendar]]"
modified: 2026-07-17
---

# Art Appreciation Semester 2 Outline

> [[classes/art-appreciation/art-appreciation|Art Appreciation]] · Semester 2 · previous: [[classes/art-appreciation/unit-plans/art-appreciation-semester-1-plan|Semester 1 Plan]]

**Status: titled outline only — T1 to T16 are not yet mapped to real dates.** Spring 2027 (≈ Feb 22 – Jun), mixed G11+G12. The semester pivots from *reading images* to *the structures of the art world* — patronage, markets, visibility — and closes on the **Atlas Book capstone**, which doubles as the S2 End-of-Term piece (no sit-down mock exists for this course).

## Unit sequence (U5–U14)

| Unit | Title |
|---|---|
| U5 | Patronage and the Contract of Belief |
| U6 | Film: *La Chimera* |
| U7 | Design of Attention |
| U8 | Visibility, Criticism and the Public |
| U9 | Galleries and the Primary Market |
| U10 | Auctions and the Secondary Market |
| U11 | Fairs, Media, Prizes and the Visibility Economy |
| U12 | Shanzhai |
| U13 | How to Make It as an Artist |
| U14 | Capstone: the Atlas Book |

## The five S2 majors

| Major | Slot |
|---|---|
| Rhetoric Breakdown | T4 |
| Pitch Package v1 | T7 |
| Catalog Entry | T9 |
| Career submission packet | T12 |
| **Atlas Book** (capstone = S2 EoT) | T16 |

## Planning rules carried in the source

1. No external paper, no mock — the anchor is the **Atlas Book**, and the spring is built backwards from it.
2. **Qingming week** (Qingming = Mon 2027-04-05; floats): a free Atlas-Book production block, not a mock. Hard-wire nothing to early April; book studio space when dates are announced.
3. May Day (≈ 05-01 → 05-05): revision / Atlas-Book production only.
4. **Confirm the S2 assessment map before the winter break** — a major in the first fortnight back must be announced before students leave in January (winter vacation 2027-01-24 → 02-21, per [[shared/school-academic-calendar]]).
5. Every S2 announcement must land on a both-cohort session; spring Juniors/Seniors disruption weeks are unknown until the spring calendar publishes.

## What's missing

Real dates for T1–T16; lesson plans; the spring school calendar (a shared source — when it arrives, sweep all courses); confirmation of how the 20% EoT category reads for the Atlas Book. The S1 master plan's lesson 15 includes handing students a one-page S2 outline — that page is among the PROPOSED items not yet confirmed.

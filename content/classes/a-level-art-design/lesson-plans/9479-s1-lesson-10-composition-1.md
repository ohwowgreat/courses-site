---
title: "9479 S1 Lesson 10: Composition 1"
tags: [a-level-art-design, cie-9479, lesson-plan, semester-1]
related:
  - "[[classes/a-level-art-design/a-level-art-design]]"
  - "[[classes/a-level-art-design/unit-plans/9479-s1-unit-4-development]]"
  - "[[classes/a-level-art-design/lesson-plans/9479-s1-lesson-09-artist-2-bridge-piece-and-a2]]"
  - "[[classes/a-level-art-design/lesson-plans/9479-s1-lesson-11-composition-2-and-critique]]"
modified: 2026-07-17
---

# 9479 S1 Lesson 10: Composition 1

[[classes/a-level-art-design/a-level-art-design|A Level Art & Design]] · [[classes/a-level-art-design/unit-plans/9479-s1-unit-4-development|Unit 4]] · Lesson 10 of 15 · ← [[classes/a-level-art-design/lesson-plans/9479-s1-lesson-09-artist-2-bridge-piece-and-a2|L09]] | [[classes/a-level-art-design/lesson-plans/9479-s1-lesson-11-composition-2-and-critique|L11]] →

## At a glance

| | |
|---|---|
| Sessions | 5 days (W11–W12) — Unit 4 opens; studio production template |
| Dates | Wed 2026-11-11 → Fri 2026-11-13, then Mon 2026-11-16 → Tue 2026-11-17 |
| AO focus | AO3 Develop |
| Deliverable | Composition 1 composed from the student's own studies, built across a two-day run, resolved and photographed |
| Feeds | Nothing collected — the homework layer carries the fortnight; CS8 collects at the end of Lesson 11 with both compositions |
| Homework | Reflection after the A2 return settles (W11); sketchbook composition studies nightly (W12) |

## Goal

Development begins with making, not talking. Composition 1 is composed from the student's own recorded studies, never from memory, then built across two uninterrupted days, resolved, and documented properly. "Unresolved work cannot be judged; badly photographed work loses real marks."

## How it runs

- **Wed 11-11** — composing from studies: tracing-paper overlays, the direction sentence taped to the board edge; compose composition 1 as a full-size cartoon or worked drawing; sign-off rota begins.
- **Thu 11-12 → Fri 11-13** — two-day build run: Thursday blocks structure and main relationships; Friday develops passages toward the whole; exit annotation each day — the decision that held the idea together.
- **Mon 11-16** — name the unresolved passages, then resolve composition 1: finish it, and declare it done in writing.
- **Tue 11-17** — documenting work: light, angle, color truth; photograph and lay out composition 1 with its trail (studies, cartoon, build shots); file to the digital portfolio.

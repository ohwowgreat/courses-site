---
title: "9607 S1 Lesson 08: Representation Applied to Your Own Product"
tags: [media-studies, cie-9607, lesson-plan, semester-1]
related:
  - "[[classes/media-studies/media-studies]]"
  - "[[classes/media-studies/unit-plans/9607-s1-unit-4-representation]]"
  - "[[classes/media-studies/lesson-plans/9607-s1-lesson-07-reading-systematically-toward-the-blog-midpoint]]"
  - "[[classes/media-studies/lesson-plans/9607-s1-lesson-09-the-gaze]]"
  - "[[classes/media-studies/9607-resource-library]]"
modified: 2026-07-21
---
# 9607 S1 Lesson 08: Representation Applied to Your Own Product

[[classes/media-studies/media-studies|Media Studies]] · [[classes/media-studies/unit-plans/9607-s1-unit-4-representation|Unit 4]] · Lesson 08 of 16 · ← [[classes/media-studies/lesson-plans/9607-s1-lesson-07-reading-systematically-toward-the-blog-midpoint|L07]] | [[classes/media-studies/lesson-plans/9607-s1-lesson-09-the-gaze|L09]] →

## At a glance

| | |
|---|---|
| Days | 6 — Mon (A2 due), Tue, Wed double, Fri, then Mon + Tue standalone |
| Dates | Mon 2026-10-26 · Tue 10-27 · Wed 10-28 · Fri 10-30 · Mon 11-02 · Tue 11-03 |
| Band/Unit | U4 Representation ([[classes/media-studies/unit-plans/9607-s1-unit-4-representation\|S1 Unit 4]]) |
| AO focus | AO3 (A2 in); representation theory toward AO2 |
| Deliverable | A2 submitted day 1; stereotype audit of your own rough cut, evidenced for LB3 |
| Feeds | A3 |
| Homework | Production pickups identified by the audit |

## What this covers

You learn representation theory and then apply it directly to your own work. The week opens with A2 going in and ends with two standalone sessions where your rough cut is screened as an audience would see it and audited for stereotypes you may not have noticed — while there is still production time to fix them.

## Session by session

- **D1 (Mon, single):** Submit your A2 blog URLs in the first five minutes. Then: representation as construction — the same person represented three ways, naming selection and combination.
- **D2 (Tue, single):** Hall's encoding/decoding — dominant, negotiated, oppositional; place your own honest reading of one text and defend the placement.
- **D3 (Wed, double):** stereotyping as power — who benefits, who pays; case work on two contrasting stereotype examples; reading positions revisited on both.
- **D4 (Fri, single):** absence and selective construction — what is not in the frame; inventory one text for its absences and what each absence achieves.
- **D5 (Mon 11-02, standalone):** rough-cut screening under audience conditions — no introductions, no apologies; complete structured response cards while watching.
- **D6 (Tue 11-03, standalone):** stereotype audit of your own product using the week's full toolkit (construction, positions, power, absence); **LB3 posted**; draft your fix list.

## Assessment

| Item | Date | What | AO |
|---|---|---|---|
| A2 | Mon 2026-10-26 | C1 blog mid-point, posts 1–7, submitted by URL | AO3 |
| LB3 | Tue 2026-11-03 | Screening participation and peer response quality | — |

See the [[classes/media-studies/assessments/9607-s1-assessments|full register]].

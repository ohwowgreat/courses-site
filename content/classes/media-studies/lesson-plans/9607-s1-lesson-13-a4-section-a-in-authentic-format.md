---
title: "9607 S1 Lesson 13: A4 — Section A in Authentic Format"
tags: [media-studies, cie-9607, lesson-plan, semester-1]
related:
  - "[[classes/media-studies/media-studies]]"
  - "[[classes/media-studies/unit-plans/9607-s1-unit-5-media-contexts]]"
  - "[[classes/media-studies/lesson-plans/9607-s1-lesson-12-institutions-and-section-a-craft]]"
  - "[[classes/media-studies/lesson-plans/9607-s1-lesson-14-consolidation-and-the-coverage-matrix]]"
  - "[[classes/media-studies/9607-resource-library]]"
modified: 2026-07-21
---
# 9607 S1 Lesson 13: A4 — Section A in Authentic Format

[[classes/media-studies/media-studies|Media Studies]] · [[classes/media-studies/unit-plans/9607-s1-unit-5-media-contexts|Unit 5]] · Lesson 13 of 16 · ← [[classes/media-studies/lesson-plans/9607-s1-lesson-12-institutions-and-section-a-craft|L12]] | [[classes/media-studies/lesson-plans/9607-s1-lesson-14-consolidation-and-the-coverage-matrix|L14]] →

## At a glance

| | |
|---|---|
| Days | 4 — Mon+Tue two-day block, Wed double (A4 sits), Fri single |
| Dates | Mon 2026-12-14 · Tue 12-15 · Wed 12-16 · Fri 12-18 |
| Band/Unit | U5 Media Contexts ([[classes/media-studies/unit-plans/9607-s1-unit-5-media-contexts\|S1 Unit 5]]) |
| AO focus | AO1 + AO2 |
| Deliverable | A4 script: Component 2 Section A — 30 min viewing and notes plus 45 min writing |
| Feeds | End of Term Exam; Forecast Grade anchor |
| Homework | None — attainment week |

These dates fall in a future term and may shift when the school publishes its calendar for that period.

## Goal

A4 is your Forecast Grade anchor. Over two days you trace one institution across ownership, distribution, audience and regulation. Wednesday is your first Section A sitting in the real format: thirty minutes of viewing and note-making before forty-five minutes of writing. This is a different skill from A1's single hour, and the briefing treats it that way. Friday adds the regulation context while the sitting is fresh.

## How it runs

- **D1–2 (Mon+Tue block):** integration, synergy and convergence defined and found in one institution's real output; on day 2, the same institution is traced end to end. You update your case-study documents live, and each day ends with a two-minute structured-notes drill.
- **D3 (Wed, double): A4 sits** — 5 min setup / 30 min viewing and notes / 45 min writing / 10 min collect. You get a briefing on what the viewing window is for, and the sitting runs exactly to the paper's clock. There is no debrief today: the format itself is the lesson.
- **D4 (Fri, single):** regulation context — who regulates the case-study institution and what that changes about its output; add dated entries to your document.

## Assessment

| Item | Date | What | AO |
|---|---|---|---|
| A4 | Wed 2026-12-16 | Component 2 Section A in authentic format, unseen TV-drama extract: 30 min viewing/notes + 45 min writing; Forecast Grade anchor | AO1+AO2 |

See the [[classes/media-studies/assessments/9607-s1-assessments|full register]].

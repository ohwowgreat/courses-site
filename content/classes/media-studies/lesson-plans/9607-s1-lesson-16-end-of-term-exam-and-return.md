---
title: "9607 S1 Lesson 16: End of Term Exam and Return"
tags: [media-studies, cie-9607, lesson-plan, semester-1]
related:
  - "[[classes/media-studies/media-studies]]"
  - "[[classes/media-studies/unit-plans/9607-s1-unit-6-revision-and-consolidation]]"
  - "[[classes/media-studies/lesson-plans/9607-s1-lesson-15-exam-rehearsal]]"
  - "[[classes/media-studies/9607-resource-library]]"
modified: 2026-07-21
---
# 9607 S1 Lesson 16: End of Term Exam and Return

[[classes/media-studies/media-studies|Media Studies]] · [[classes/media-studies/unit-plans/9607-s1-unit-6-revision-and-consolidation|Unit 6]] · Lesson 16 of 16 · ← [[classes/media-studies/lesson-plans/9607-s1-lesson-15-exam-rehearsal|L15]]

## At a glance

| | |
|---|---|
| Days | 2 teaching days + exam window |
| Dates | Exam sat in window Mon 2027-01-11 → Mon 01-18 · Tue 01-19 return · Wed 01-20 double, goal setting |
| Band/Unit | U6 Revision ([[classes/media-studies/unit-plans/9607-s1-unit-6-revision-and-consolidation\|S1 Unit 6]]) |
| AO focus | AO1 + AO2 (full paper); AO-level gap analysis after |
| Deliverable | End of Term Exam script (20% category); one-page Semester 2 target sheet per student (LB7) |
| Homework | None |

These dates and your exact exam slot may shift when the school publishes the exam calendar; the confirmed slot will be given in class.

## What happens

You sit the full Component 2 paper (2 hours, exam venue, full conditions) in the window Mon 01-11 → Mon 01-18. Your exact slot within that window is set by the exam office. Scripts come back on Tuesday with question-level AO feedback. On the Wednesday double you turn that feedback into a one-page Semester 2 target sheet: where your marks went, which AO is the gap, and what Semester 2 does about it. Grades and comments are issued the same Wednesday.

## How it runs

- **Exam (window Mon 01-11 → Mon 01-18):** full Component 2, 2 hours, exam venue, full conditions.
- **D1 (Tue 01-19, single):** scripts returned with question-level AO breakdowns; assessment review — self-assessment before marks, gaps named in writing.
- **D2 (Wed 01-20, double):** target sheets built from the AO data — gap, evidence, action, review date; **LB7 posted** (response to feedback, S2 goal setting); Semester 2 preview — coursebook units U6–U8, C2 Section B depth. **Grades and comments issued today.**

## Assessment

| Item | Date | What | AO |
|---|---|---|---|
| EoT | window Mon 2027-01-11 → Mon 01-18 | Full Component 2 paper, 2 hours, exam venue; 20% End-of-Term category | AO1+AO2 |
| LB7 | Wed 2027-01-20 | Response to feedback; Semester 2 goal setting | — |

See the [[classes/media-studies/assessments/9607-s1-assessments|full register]].
